postanalysis.pipeline<- function(configFile){
  ...
  else message("Using all subjects for LD calculation.")
  ...
  chunkMb<- 4 #7 #4
  dose.all <- do.call(cbind, lapply(1:length(varlist.all), function (idx){
    ...
    if (idx ==1) return (curr$dose)
    else return (curr$dose[-1])
  })) 
  ...
  for( idx in 1:nrow(models)){
    ...
    gres <- lapply(agroups, function(group){
      curr<- padata.permodelgroup(paste("analyses",model, group,"ALL.out.txt.gz", sep ="/" ),  
                                        varlist, flanking,  threshold.MAF, threshold.Rsq)
      for(c in c("pvalue", "beta", "SE",  "analysed.Freq1", "analysed.Rsq", "MAF")) 
    ...
    assoc <- Reduce(function(x, y) merge(x, y, all = T, by = c( "SNP","Al1", "Al2","CHROM", "POS")), 
    ...
    if(plot) 
      lapply(varlist, function(snp){
        ...
        if(nrow(assoc.curr[snp,nomatch=0])>0 &nrow(assoc.curr) > 0) {
          ...
          col.pvals<- names(assoc.curr)[grep("pvalue", names(assoc.curr))]
          #region plot
          ...
          regionplot.multiP (assoc.curr, chr, pos, flanking = 250, col.r2="R2_LD",
                            col.pvals = col.pvals, plabel = gsub(".","/", gsub("pvalue.","", col.pvals, fixed = T), fixed = T), 
          ...
          lapply(agroups, function(group){
            ...
            se<-unlist(assoc.curr[snp, paste("SE", group, sep ="."),with = FALSE])
            ci<-beta+c(-1, 1) *qnorm(0.975)*se
            main <- paste(model," in ",group, ": ", snp, sep ="")
            main.sub<- paste(c("Freq1", "Rsq_imp","P"),  
                             format(assoc.curr[snp, paste(c("analysed.Freq1", "analysed.Rsq", "pvalue"), group, sep ="."), with = FALSE], digit = 3), 
                             sep = "=", collapse = ", ")
            if(detail$type == "qt") {
              qtplot(detail$pheno, snp, data.plot, xlab = NULL)
              main.sub<- paste(main.sub,  ", eff(CI)=", format(beta, digit = 2), "(",
                               paste(format(ci, digit = 2), collapse =","), ")", sep ="")
            }
            if(detail$type %in% c("binary", "ordinal")) {
              otplot(detail$pheno, snp, data.plot, xlab = NULL)
              main.sub<- paste(main.sub,  ", OR(CI)=", format(exp(beta), digit = 2), "(",
            ...
            if(detail$type == "survival") {
              ...
              main.sub<- paste(main.sub,  ", HR(CI)=", format(exp(beta), digit = 2), "(",
                               paste(format(exp(ci), digit = 2), collapse =","), ")", sep ="")
            }
            title(main, line = 2)  
          ...
          if(length(acontrasts)>0){
            contrastP<-unlist(assoc.curr[snp, paste("contrastpvalue.", gsub("/",".",acontrasts, fixed = T), sep =""), with = FALSE])
            mtext(paste("P.",acontrasts, "=", format(contrastP, digit = 3), sep ="", collapse =", "), 
...
padata.permodelgroup <- function(retFile, varlist, flanking,  threshold.MAF, threshold.Rsq){
  ...
  res<- getResults(retFile, varlist, flanking )
...
getResults<- function(retFile, varlist, flanking ){
  ...
  stopifnot(all(c("X.CHROM", "POS", "SNP", "pvalue", "beta", "SE", "Al1", "Al2", "analysed.Freq1", "analysed.Rsq") %in% names(res1)))
